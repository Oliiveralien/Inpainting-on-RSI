...
def validate(netG, netD, loss_terms, optG, optD, dataloader, epoch, device=cuda0, batch_n="whole", save_pic_freq=10):
    ...
    for i, (imgs, masks) in enumerate(dataloader):
        ...
        if i+1 < config.STATIC_VIEW_SIZE:
            ...
            fid_score = metrics['fid']([path1, path2], cuda=True)  #numpy.float64
            ssim_score = metrics['ssim']([path1, path2])  #tensor
            ...
            tensorboardlogger.scalar_summary('val/psnr', psnr_score.item(), epoch*len(dataloader)+i)
            break
